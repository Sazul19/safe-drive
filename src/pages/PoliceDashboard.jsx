...
export default function PoliceDashboard({ onLogout }) {
  ...
  useEffect(() => {
    ...
    startLocationWatch((loc) => {
      ...
      const activeAlert = list.find(a => a.policeStatus === 'en_route')
      const arrivedAlert = !activeAlert && list.find(a => a.policeStatus === 'arrived')
      ...
      if (activeAlert) {
        ...
        updateUnitLocation(user.uid, 'police', loc.lat, loc.lng, activeAlert.id, startCoordsRef.current.lat, startCoordsRef.current.lng)
      } else if (arrivedAlert) {
        if (activeAlertIdRef.current !== arrivedAlert.id) {
          activeAlertIdRef.current = arrivedAlert.id
        }
        updateUnitLocation(user.uid, 'police', loc.lat, loc.lng, arrivedAlert.id, startCoordsRef.current.lat, startCoordsRef.current.lng)
      } else {
